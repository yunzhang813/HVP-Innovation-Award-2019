...
The __goal__ of this analysis is to identify differentially expressed genes (DEG) between responders and non-responders in the HVP01 study using the _bulk_ RNA-seq data. 
...
In this report, we __focus__ on the _response after dose 2_ (with 13 responders and two non-responders). At a single time point, we perform a naive t-test. To include multiple time points, we also perform the parametric "PBtest", which takes into account the repeated measurement data structure. For more about the `PBtest`, please read [here](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-019-2783-8).
...
Earlier, we read in "HVP_sampleTable_response_May18.csv", which contains the dose response and other clinical information for each sample in the expression matrix.
